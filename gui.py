...
# core.pyのRakutenProductFinderクラスをインポート
try:
    from core import RakutenProductFinder
except ImportError:
    print("エラー: core.py が同じディレクトリに見つかりません。")
    exit()
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(settings_frame, text="列 (JANコード/出力開始):").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Label(col_frame, text="(商品名,価格,URLの順に出力)").pack(side=tk.LEFT, padx=10)
    ...
    def run_in_thread(self, config):
        ...
        try:
            finder = RakutenProductFinder(config, logger_callback=self.log)
            finder.run_process()
